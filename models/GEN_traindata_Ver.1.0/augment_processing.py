...
def return_block_size(input_dict):
    ...
    if gv.FULL_SIZE <= init_gap:
        gap_full_gap = (init_gap-gv.FULL_SIZE)
        temp_end = gap_full_gap//2
        temp_start = gap_full_gap-temp_end

        new_start = init_start+temp_start
        new_end = init_end-temp_end
        new_gap = new_end-new_start

        input_dict['start_index'] = new_start
        input_dict['end_index'] = new_end
        input_dict['gap_start_end'] = new_gap

    init_start = input_dict['start_index']
    init_end = input_dict['end_index']
    init_gap = input_dict['gap_start_end']

    temp_gap = gv.FULL_SIZE-init_gap
    temp_a = temp_gap//gv.DATA_AUG_POSITION

    if temp_a <= gv.AUG_THRES_SIZE:
        block_size = gv.AUG_THRES_SIZE
        aug_num = temp_gap//block_size
    else:
        aug_num = gv.DATA_AUG_POSITION
        block_size = temp_gap//aug_num
