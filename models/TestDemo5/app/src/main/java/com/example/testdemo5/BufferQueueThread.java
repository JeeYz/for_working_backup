...
public class BufferQueueThread implements Runnable {
    ...
    private TextView fromMainTextView;
    private ImageView fromMainImageView;

    private Handler decoHandler = new Handler();


    private int toProcessingDataNum = 0;

    public BufferQueueThread(AudioRecord audioRec,
                             GlobalVariablesClass globalVariables,
                             RecordingData recordingDataClass,
                             Handler fromDecoClass) {
...
//        this.bufferQueueData = new ArrayList<>();
        ...
        this.fromMainTextView = globalVariables.getPtrMainTextView();
        this.fromMainImageView = globalVariables.getMicImageView();

        this.decoHandler = fromDecoClass;
...
//                    Log.d("Mean Value", String.valueOf(meanFloat));
                    ...
                    if (meanFloat > this.voiceTriggerValue || getToProcessingDataNum() > 0) {
                        ...
                        if (getToProcessingDataNum() == 0){
                            Message msg = Message.obtain(this.decoHandler);
                            msg.what = 0;
                            msg.arg1 = this.globalVariables.getStartSaveData();
                            this.decoHandler.sendMessage(msg);
                        }
...
//                            audioRec.release();
            ...
            e.printStackTrace();
            Log.d("status", "녹음이 정상적으로 동작하지 않습니다.11");
        }
    }
