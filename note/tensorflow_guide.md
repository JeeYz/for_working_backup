...
adapt() 메서드는 numpy 배열 또는 tf.data.Dataset 객체 사용


## 모델 전 또는 모델 내부의 데이터 전처리

옵션 1. 전처리 레이어를 모델의 일부로 만듦.

옵션 2. tf.data.Dataset에 전처리 레이어를 적용하여 다음과 같이 전처리 된 데이터의 배치를 생성하는 데이터세트를 얻는다.

## 추론 시 모델 내에서 전처리를 수행할 때의 이점

옵션 2를 사용하더라도 나중에 전처리 레이어를 포함하는 추론 전용 엔드 투 엔드 모델을 내보낼 수 있다.
이 작업의 주요 이점은 모델을 이식 가능하게 만들고 훈련/적용 편향을 줄이는 데 도움이 된다.
...
지도 학습을 수행할 때 fit()를 사용
훈련 루프를 처음부터 작성해야 하는 경우, GradientTape를 사용하여 모든 세부 사항을 제어할 수 있음.

Keras의 핵심 원칙은 복잡성의 점진적인 공개
** 항상 점진적으로 저수준 워크플로부터 시작할 수 있어야 함. **
적절한 수준의 고수준 편의를 유지하면서 작은 세부 사항을 보다 효과적으로 제어할 수 있어야 함.

fit()를 사용자 정의해야 하는 경우, Model 클래스의 훈련 단계 함수를 재정의 해야 함.
이 함수는 모든 데이터 배치에 대해 fit()에 의해 호출되는 함수


## 첫번째 간단한 예제

self.compiled_loss를 통해 손실을 계산하여 compile()로 전달된 손실 함수를 래핑
...
Keras는 기본 학습 및 평가 루프인 fit() 및 evaluate()를 제공.

fit()의 편리성을 이용하면서 해당 모델의 학습 알고리즘을 사용자 정의 하려면 Model 클래스를 하위 클래스화 하고 fit() 중에 반복적으로 호출되는 고유 train_step()메서드를 구현

## GradientTape 사용하기 : 첫 번째 엔드 투 엔드 예제

GradientTape 범위 내에서 모델을 호출하면 손실 값과 관련하여 레이어의 훈련 가능한 가중치 기울기를 가져올 수 있다.
최적화 프로그램 인스턴스를 사용하면 이러한 기울기를 이용해 이러한 변수를 업데이트

사용자 정의 훈련 루프
* for 루프를 엶
* 각 에폭마다 데이터 집합을 반복적으로 처리하는 for 루프를 엶
* 각 배치마다 GradientTape()범위를 엶
* 이 범위 내에서 모델을 호출하고 손실을 계산
* 스코프 외부에서 손실과 관련하여 모델 가중치의 기울기를 검색
* 마지막으로, 그라디언트를 기반으로 모델의 가중치를 업데이트하기 위해 옵티 마이저를 사용
...
계산을 정적 그래프로 정의하면 프레임 워크에서 전체 성능 최적화를 적용
텐서를 입력으로 사용하는 모든 함수를 정적 그래프로 컴파일 할 수 있음.
...
RNN은 시계열 또는 자연어와 같은 시퀀스 데이터를 모델링하는 신경망 클래스

## 내장 RNN 레이어: 간단한 예

## 출력과 상태

출력 형상은 (batch_size, units)이고, 여기서 units는 레이어의 생성자에 전달된 units 인수에 해당
return_sequences=True를 성정하면 RNN 레이어가 각샘플에 대한 전체 출력 시퀀스도 반환 -> (batch_size, timesteps, units)
...
tensorflow 2.0에서 내장 LSTM 및 GRU 레이어는 GPU를 사용할 수 있을 때 기본적으로 CuDNN 커널을 활용하도록 업데이트 되었다.
CuDNN 커널은 특정한 가정 하에 구축되므로 다음과 같이 내장 LSTM 또는 GRU 레이어의 기본값을 변경하면 레이어가 CuDNN 커널을 사용할 수 없게 된다.
...
모든 샘플의 길이가 균일하므로 데이터의 일부가 실제로 채워져 있다는 사실을 모델에 알려야 한다.

Keras 모델에서 입력 마스크를 도입하는 세 가지 방법
...
## 마스킹 생성 레이어 : Embedding 및 Masking
...
1. 이전에 훈련된 모델에서 레이어를 가져옴
2. 추후 훈련 라운드 중에 포함된 정보가 손상되지 않도록 고정
3. 고정된 레이어 위에 훈련할 수 있는 새 레이어를 추가. 해당 레이어는 기존 기능을 새로운 데이터 세트에 대한 예측으로 전환
4. 데이터 세트에서 새로운 레이어를 훈련
...
모델 또는 하위 레이어가 있는 레이어에서 trainable = False를 설정 하면 모든 하위 레이어도 훈련할 수 없다.
...
모델이 새로운 데이터에 수렴하면 기본 모델의 일부 또는 전부를 동결 해제하고 학습 속도가 매우 낮은 전체 모델을 전체적으로 재훈련할 수 있다.

## 사용자 지정 훈련 루프를 사용한 전이 학습 및 fine-tuning

## 엔드 투 엔드 예제: 고양이 vs 개 데이터 세트에서 이미지 분류 모델 미세 조정
...
## 전체 모델의 fine-tuning을 수행

### 데이터 입력 파이프라인

### tf.data: tensorflow 입력 파이프 라인 빌드

tf.data API를 사용하면 간단하고 재사용 가능한 부분에서 복잡한 입력 파이프 라인을 구축할 수 있다.
이미지 모델의 파이프 라인은 분산 파일 시스템의 파일에서 데이터를 집계하고, 각 이미지에 무작위 섭동을 적용하고, 무작위로 선택한 이미지를 훈련을 위해 배치로 병합 할 수 있다.
텍스트 모델의 파이프 라인에는 원시 텍스트 데이터에서 기호를 추출하고, 조회 테이블이 있는 포함 식별자로 변환하고, 서로 다른 길이의 시퀀스를 일괄 처리하는 작업이 포함될 수 있다.
tf.data API를 사용하면 많은 양의 데이터를 처리하고 다양한 데이터 형식에서 읽고 복잡한 변환을 수행할 수 있다.

tf.data API는 각 요소가 하나 이상의 구성 요소로 구성된 일련의 요소를 나타내는 tf.data.Dataset 추상화를 도입한다.
...
메모리의 Dataset에서 데이터 Dataset를 생성하려면 tf.data.Dataset.from_tensors() 또는 tf.data.Dataset.from_tensor_slices()사용할 수 있다.

# 데이터 세트 구조

리스트 입력을 구조로 처리하려면 이를 튜플로 변환해야 하며 리스트 출력이 단일 구성 요소가 되도록 하려면 tf.stack을 명시적으로 tf.stack해야 한다.
